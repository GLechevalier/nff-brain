...
export function pruneBrain(brain: BrainFile, maxTotalNodes: number): number {
  // graphify/supabase imports don't count toward the budget (they're bounded at
  // ingest time and replaced wholesale on re-ingest) and are never eviction
  // victims. Clip nodes likewise: they have their own cap (pruneClips) and must
